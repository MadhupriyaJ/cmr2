    ...
    public class DesignationManagementController : Controller
    {
        ...
        public ActionResult Index(string sortOrder, string currentFilter, string searchString, int? page)
        {
            try
            {
                ...
                ViewBag.Module = client.GetDesignLst(Login.OrganizationId);
                Core.Basic.TimeLineDescription(this.ControllerContext.RouteData.Values["controller"].ToString(),
                    this.ControllerContext.RouteData.Values["action"].ToString(), "The user " + Login.FirstName + " " + Login.LastName + "Entered into List of Designation Page.");
                return View();
            }
            catch (Exception ex)
            {
        ...
        public string TableData()
        {
            try
            {
                ...
                var LstDesign = client.GetDesignLst(Login.OrganizationId);

                totalData = LstDesign.Count();
                if (!string.IsNullOrEmpty(search))
                    LstDesign = LstDesign.Where(a => a.Name.Contains(search)).ToArray();
                if (!string.IsNullOrEmpty(order))
                {
                    switch (order)
                    {
                        case "1":
                            LstDesign = (orderBy == "asc" ? LstDesign.OrderBy(a => a.Name) : LstDesign.OrderByDescending(a => a.Name)).ToArray();
                            break;
                        case "2":
                            LstDesign = (orderBy == "asc" ? LstDesign.OrderBy(a => a.CreateDate) : LstDesign.OrderByDescending(a => a.CreateDate)).ToArray();
                            break;
                        case "3":
                            LstDesign = (orderBy == "asc" ? LstDesign.OrderBy(a => a.ModifiedDate) : LstDesign.OrderByDescending(a => a.ModifiedDate)).ToArray();
                            break;
                        default:
                            LstDesign = (orderBy == "asc" ? LstDesign.OrderBy(a => a.Id) : LstDesign.OrderByDescending(a => a.Id)).ToArray();
                            break;
                    }
                }
                else
                    LstDesign = LstDesign = (orderBy == "asc" ? LstDesign.OrderBy(a => a.Id) : LstDesign.OrderByDescending(a => a.Id)).ToArray();
                if (!String.IsNullOrEmpty(page) && !string.IsNullOrEmpty(displaylimit) && displaylimit != "-1")
                {
                    try
                    {
                        LstDesign = LstDesign.Skip(Convert.ToInt32(!String.IsNullOrEmpty(page) ? page : "0")).Take(Convert.ToInt32(displaylimit)).ToArray();
                    }
                    catch { }
                }
                ...
                string aa = "{";
                aa += "\"recordsTotal\": " + totalData + ", \"recordsFiltered\": " + LstDesign.Count() + ",\"data\":[";
                int temp = 0;
                foreach (var item in LstDesign)
                {
                    if (temp > 0)
                        aa += ",";
                    temp++;
        ...
        {
            Core.Basic.TimeLineDescription(this.ControllerContext.RouteData.Values["controller"].ToString(),
                   this.ControllerContext.RouteData.Values["action"].ToString(), "The user " + Login.FirstName + " " + Login.LastName + "Entered into Designation Create Page.");
            return View();
        }
        #endregion
        ...
        [LoginAuthorize]
        public bool AddDesignation(String Designation)
        {
            try
            {
                if (!String.IsNullOrEmpty(Designation))
                {
                    FramWork.HelloService.Designation tbl = new FramWork.HelloService.Designation();
                    tbl.Id = Guid.NewGuid();
                    tbl.Name = Designation;
                    tbl.ModifiedBy = Login.Id;
                    tbl.OrgId = Login.OrganizationId;
                    HelloServiceClient client = new HelloService.HelloServiceClient("BasicHttpBinding_IHelloService");
                    client.AddDesignation(tbl);
                    Core.Basic.TimeLineDescription(this.ControllerContext.RouteData.Values["controller"].ToString(),
                 this.ControllerContext.RouteData.Values["action"].ToString(), "The user " + Login.FirstName + " " + Login.LastName + "Added Designation.");
                }
                else
                {
