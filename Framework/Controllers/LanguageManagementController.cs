...
this.ControllerContext.RouteData.Values["action"].ToString(), "The user " + Login.FirstName + " " + Login.LastName + "Entered into List of Langugae Page.");
            return View();
        }
        #endregion
        ...
        public string TableData()
        {
            try
            {
                ...
                var LstLang = client.GetLanguageLst(Login.OrganizationId);

                totalData = LstLang.Count();
                if (!string.IsNullOrEmpty(search))
                    LstLang = LstLang.Where(a => a.Name.Contains(search)).ToArray();
                if (!string.IsNullOrEmpty(order))
                {
                    switch (order)
                    {
                        case "1":
                            LstLang = (orderBy == "asc" ? LstLang.OrderBy(a => a.Name) : LstLang.OrderByDescending(a => a.Name)).ToArray();
                            break;
                        case "2":
                            LstLang = (orderBy == "asc" ? LstLang.OrderBy(a => a.CreateDate) : LstLang.OrderByDescending(a => a.CreateDate)).ToArray();
                            break;
                        case "3":
                            LstLang = (orderBy == "asc" ? LstLang.OrderBy(a => a.ModifiedDate) : LstLang.OrderByDescending(a => a.ModifiedDate)).ToArray();
                            break;
                        default:
                            LstLang = (orderBy == "asc" ? LstLang.OrderBy(a => a.Id) : LstLang.OrderByDescending(a => a.Id)).ToArray();
                            break;
                    }
                }
                else
                    LstLang = LstLang = (orderBy == "asc" ? LstLang.OrderBy(a => a.Id) : LstLang.OrderByDescending(a => a.Id)).ToArray();
                if (!String.IsNullOrEmpty(page) && !string.IsNullOrEmpty(displaylimit) && displaylimit != "-1")
                {
                    try
                    {
                        LstLang = LstLang.Skip(Convert.ToInt32(!String.IsNullOrEmpty(page) ? page : "0")).Take(Convert.ToInt32(displaylimit)).ToArray();
                    }
                    catch (Exception ex)
                    {
                ...
                string aa = "{";
                aa += "\"recordsTotal\": " + totalData + ", \"recordsFiltered\": " + LstLang.Count() + ",\"data\":[";
                int temp = 0;
        ...
        {
            Core.Basic.TimeLineDescription(this.ControllerContext.RouteData.Values["controller"].ToString(),
this.ControllerContext.RouteData.Values["action"].ToString(), "The user " + Login.FirstName + " " + Login.LastName + "Entered into Create Language Page.");
            return View();
        }
        #endregion
        ...
        public bool AddLanguage(String Language)
        {
            try
            {
                ...
                {
                    FramWork.HelloService.Language tbl = new FramWork.HelloService.Language();
                    tbl.Id = Guid.NewGuid();
                    tbl.Name = Language;
                    tbl.ModifiedBy = Login.Id;
                    tbl.OrgId = Login.OrganizationId;
                    HelloServiceClient client = new HelloService.HelloServiceClient("BasicHttpBinding_IHelloService");
                    client.AddLanguage(tbl);
                    Core.Basic.TimeLineDescription(this.ControllerContext.RouteData.Values["controller"].ToString(),
this.ControllerContext.RouteData.Values["action"].ToString(), "The user " + Login.FirstName + " " + Login.LastName + "Added Language.");
                }
                else
                {
