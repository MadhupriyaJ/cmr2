    ...
    return {
        ...
        init: function () {
            ...
            form.validate({
                ...
                rules: {
                    RPnumber: {required: true},
                    Passport: { required: true},
                    PersonModeId: {required: true},
                    Salutation: {required: true},
                    FirstName: {required: true},
                    LastName: {required: true},
                    Language: { required: true },
                    Salary: { required: true },
                    CurrentLiablility: { required: true },
                    MonthlyInstallment: { required: true },
                    ExpectedValue: { required: true },
                    ddlassignedtoteam: {},
                    ddlassignedtoemployee: {},
                    ExpectedClosingDate: { required: true },
            ...
            $('#form_wizard_1 .button-submit').click(function () {
                ...
                var PersonData = {
                    ...
                    'LanguageId': $('select[name="LanguageId"] option:selected').val(),
                ...
                var PersonLeadData = {
                    ...
                    'Salary': $('#Salary').val(),
                    'CurrentLiablility': $('#CurrentLiablility').val(),
                    'MonthlyInstallment': $('#MonthlyInstallment').val(),
                    'ExpectedValue': $('#ExpectedValue').val(),
                    'ExpectedClosingDate': $('#ExpectedClosingDate').val(),
                    'Assignedto': $('select[name="ddlassignedtoemployee"] option:selected').val()
                };

                //var dataRow = ({ PersonTbl: PersonData, PersonLeadTbl: PersonLeadData });
                //var dataRow = PersonLeadData;
                $.ajax({
                    type: 'POST',
                    url: '../PersonManagment/AssignAsLead',
                    data: PersonLeadData,
                    success: function (response) {
                        if (response == 'True') {
                            bootbox.alert("Assigned As Lead Successfully!", function () {
                                window.location.href = "../SalesManagement/SalesAgentView";
                            });
                        }
                        else {
